...
function enterGuess() {
  ...
  function getStore() {
    return JSON.parse(window.localStorage.getItem("places_lo") || "{}");
  }
  
  function setStore(places) {
    window.localStorage.setItem("places_lo", JSON.stringify(places));
  }
  ...
  App.prototype.displayScore = function () {
    var score = Object.keys(
      JSON.parse(window.localStorage.getItem("places_lo") || "{}")
    ).length;
  ...
  App.prototype.resetGame = function () {
    // Clear local storage
    window.localStorage.removeItem("places_lo");
