...
function enterGuess() {
  ...
  function getStore() {
    return JSON.parse(window.localStorage.getItem("places_dce") || "{}");
  }
  
  function setStore(places) {
    window.localStorage.setItem("places_dce", JSON.stringify(places));
  }
  ...
  App.prototype.displayScore = function () {
    var score = Object.keys(
      JSON.parse(window.localStorage.getItem("places_dce") || "{}")
    ).length;
  ...
  App.prototype.winMessage = function () {
    alert("Well done! You found all " + this.placeList.length + " DLR, Cable Car and Elizabeth Line stations.");
  };
  ...
  App.prototype.resetGame = function () {
    // Clear local storage
    window.localStorage.removeItem("places_dce");
